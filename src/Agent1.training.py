...
maxReplayBufferSize = 50
max_episode_length = 100
updateT = 1
nbrStepsPerReplay = 10
...
if useGAZEBO :
	...
	model_path = './model-GazeboRL-robot1swarm+ExplorationNoise001'
	...
	def build_loss_functions(self):
		with tf.variable_scope(self.scope) :
			...
			if self.scope != 'global':
				...
				# MINIMIZATION OF THE REWARD(PENALTY..) :
				self.policy_loss = -tf.reduce_sum(self.Qvalue)
				self.loss = 0.5 * self.Qvalue_loss + self.policy_loss - self.entropy * 0.01 + self.lambda_regL2*self.l2_loss
