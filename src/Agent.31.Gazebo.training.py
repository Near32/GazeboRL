...
model_path = './testmulti-31-'+str(updateTauTarget)
#model_path = './testmultiCPU'
...
class AC_Network():
	...
	def build_actor(self, inputs, keep_prob, phase, scope) :
		...
		with tf.variable_scope(scope):
			...
			scaled_out = 	self.nn_layer(rnn_out, shape_out[1], self.a_size, 'policy', act=tf.tanh, std=1e-3, uniform=True)	
			#scaled_out = 	self.nn_layerBN(rnn_out, shape_out[1], self.a_size, phase, 'policy', act=tf.tanh, std=None)	
	...
	def build_critic(self, inputs, keep_prob, phase, scope) :			
		...
		with tf.variable_scope(scope):
			...
			hiddenV = self.nn_layer(Vvalue, self.nbrOutput, 1, 'V-value-Advantage', act=tf.identity, std=1e-4,uniform=True)	
			hiddenA = self.nn_layer(concat, concat_shape[1], 1, 'Action-Advantage', act=tf.identity, std=1e-4,uniform=True)	
			...
			Qvalue = sumhidden #self.nn_layer( sumhidden, 1, 1, 'Q-value', act=tf.identity, std=1e-3,uniform=True)	
				...
				try :
					...
					if True : #self.number == 0 :
						...
						while d == False :
							...
							scale = self.local_network.a_bound/10.0
							theta = 0.15
							sigma = 0.3
