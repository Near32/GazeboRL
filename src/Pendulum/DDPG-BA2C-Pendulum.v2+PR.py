...
class AC_Network():
	...
	def build_actor(self, convnet, keep_prob, phase, scope) :			
		with tf.variable_scope(scope):
			# ACTOR :
			out1 = 256
			hidden1 = self.nn_layerBN(convnet, self.s_size, out1, phase, 'actor_layer1', act=tf.nn.relu, std=None)
			...
			'''
			yactor = self.nn_layerBN(dropped1, out1, self.nbrOutput, phase,'actor_layerOutput', act=tf.nn.relu, std=None)
			...
			scaled_out = 	self.nn_layerBN(rnn_out, shape_out[1], self.a_size, phase, 'policy', act=tf.tanh, std=1e-4)	
...
class Worker():
	def __init__(self,master_network,game,replayBuffer,name, model_path, global_episodes, rec=False, updateT=1, nbrStepPerReplay=64, useGAZEBO=True):
		...
		self.trainer = { 'actor':tf.train.RMSPropOptimizer(learning_rate=lr,decay=0.9,momentum=0.0,epsilon=1e-10), 'critic':tf.train.RMSPropOptimizer(learning_rate=lr*10.0,decay=0.9,momentum=0.0,epsilon=1e-10)}
				...
				try :
					...
					if self.number == 0 :
						...
						while d == False :
							...
							scale = self.master_network.a_bound/10.0
							theta = 0.15
							sigma = 0.3
							...
							episode_buffer.append([s,a,r,s1,d,q[0],0.0])
						...
						# TAKE CARE OF THAT NEW EPISODE :
						a1 = None	
						rollout = np.vstack( episode_buffer )
						...
						print('sorting...')
						#SORTING :
						self.rBuffer = sorted(self.rBuffer, key=lambda el : el[6])
					...
					if len(self.rBuffer) > self.nbrStepPerReplay*4:
						a1 = None
						...
						#idxSteps = range(-self.nbrStepPerReplay,0)
						idxSteps = [ np.random.randint(low=-self.nbrStepPerReplay*4,high=0 ) for idx in range(self.nbrStepPerReplay) ]
						...
						#UPDATE :
						for idxv,idx in enumerate(idxSteps) :
							self.rBuffer[-idx][6] = v_l[idxv]
						...
						if self.number == 0 :
							...
							summary.value.add(tag='Losses/Value Loss', simple_value=float(np.mean(v_l)))
