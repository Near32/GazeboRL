...
if useGAZEBO == False :
	s_size = 3*nbrskipframe
	
h_size = 256

a_size = 1
#model_path = './DDPG-BA2C-batch128-tau1e-3-lr1e-4-w4'
model_path = './DDPG-BA2C-batch8-tau1e-3-lr1e-4-w4'
...
class Worker():
	...
	def work(self,max_episode_length,gamma,sess,coord,saver):
		...
		s1 = np.zeros( (1,self.master_network.s_size) )
				...
				try :
					...
					if self.number == 0 :
						if not useGAZEBO :
							...
							s = self.env.reset()
							self.env.render()
							s = np.reshape(s,(-1,self.master_network.s_size,1))
							#s = process_frame(s)
						else :
							s = self.env.reset()
						...
						a_noise = 0.0
						frame_count = 1
						a = np.zeros( (1,self.master_network.a_size) )
						...
						while d == False :
							...
							if frame_count >= self.master_network.nbrskipframe :
								frame_count = 0	
								'''
								...
								summary_count += 1
								'''
								s = np.concatenate( [episode_frames[-i-1] for i in range(self.master_network.nbrskipframe) ], axis=2).reshape( (1,-1) ) 
								...
								scale = self.master_network.a_bound/20.0
								theta = 0.15
								sigma = 0.3
							...
							if useGAZEBO :
								sp, r, d, _ = envstep(self.env, a[0])
							else :
								sp, r, d, _ = self.env.step(a)
								self.env.render()

							if d == False:
								if useGAZEBO :
									sp = preprocess(sp, img_size[0], img_size[1] )
								else :
									sp = np.reshape(sp,(-1,self.master_network.s_size,1))
							else:
								sp = episode_frames[-1]
								
							episode_frames.append(sp)
									
							r /= reward_scaler
							
							
							if frame_count == 0 :#>= self.master_network.nbrskipframe :
								s = np.concatenate( [episode_frames[-i-2] for i in range(self.master_network.nbrskipframe) ], axis=2).reshape( (1,-1) ) 
								#print('State shape q : {}'.format(s.shape) )
								q = self.master_network.predict_critic(sess,s,a)
								episode_values.append(q[0])
								#print('frame count 0 : concat :')
								s1 = np.concatenate( [episode_frames[-i-1] for i in range(self.master_network.nbrskipframe) ], axis=2).reshape( (1,-1) ) 
								episode_buffer.append([s,a,r,s1,d,q[0]])
								s = episode_frames[-1]
								#print('frame count 0 : CLEAN')
								#restart the frame buffer
						...
						print('MEAN EXECUTION TIME : {}, done: {}'.format((end - start)/(max_episode_length-remainingSteps),d) )
