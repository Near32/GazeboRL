...
from probabilityDensity import *
...
if useGAZEBO :
	a_size = 2	
	model_path = './A3C-r1s-'+'w'+str(num_workers)+'-lr'+str(lr)+'-b'+str(nbrStepsPerReplay)+'-T'+str(updateT)+'-tau'+str(updateTauTarget)+'-skip'+str(nbrskipframe)
	if threadExploration :
		model_path = model_path+'+TheadExploration'
	if fromState :
		model_path = model_path+'+FromState'
else :	
	model_path = './Stochastic-'+'w'+str(num_workers)+'-lr'+str(lr)+'-b'+str(nbrStepsPerReplay)+'-T'+str(updateT)+'-tau'+str(updateTauTarget)+'-skip'+str(nbrskipframe)
...
class AC_Network():
	def __init__(self,imagesize,s_size,h_size, a_size,a_bound,scope,trainer,tau=1e-2,rec=False,dropoutK=1.0,useGAZEBO=False, fromState=False, useVvalue=True, useSharedCNN=True, stochastic=True):
		self.stochastic = stochastic
		self.useGAZEBO = useGAZEBO
		self.fromState = fromState
		self.useVvalue = useVvalue
		self.useSharedCNN = useSharedCNN
		...
		if self.stochastic :
			self.build_stochastic_loss_functions()
		else :
	...
	def build_actor(self, inputs, keep_prob, phase, scope) :
		...
		with tf.variable_scope(scope):
			...
			if self.stochastic :
				mean = 	self.nn_layer(rnn_out, shape_out[1], self.a_size, 'policy_mean', act=tf.identity, std=1e-3, uniform=False)	
				sigma = 1e-5+self.nn_layer(rnn_out, shape_out[1], self.a_size, 'policy_sigma', act=tf.nn.softplus, std=1e-3, uniform=False)	
				#scaled_out = 	self.nn_layerBN(rnn_out, shape_out[1], self.a_size, phase, 'policy', act=tf.tanh, std=1e-6, uniform=False)	
				pd = DiagGaussianPd(mean, sigma)
				action = pd.sample()
				policy = (action,pd)	
			else :
	...
	def build_loss_functions(self):
		with tf.variable_scope(self.scope) :
			...
			#Gradients :
			if self.useVvalue :
				valuereshaped = tf.reshape(self.Vvalue,(-1,1)) 
			else :
				valuereshaped = tf.reshape(self.Qvalue,(-1,1))
			
			self.value_loss = tf.reduce_mean(tf.abs(self.target_value - valuereshaped))
	...
	def build_stochastic_loss_functions(self):
		with tf.variable_scope(self.scope) :
			#Only the worker network need ops for loss functions and gradient updating.
			#PLACEHOLDER :
			self.target_value = tf.placeholder(shape=[None,1],dtype=tf.float32,name='target_value')
			...
			#Actor :
			#
			self.policy_action = self.policy[0]
			self.policy_pd = self.policy[1]
			#Target network... :
	...
	def predict_actor(self, sess, inputs, phase=False) :
		...
		if self.stochastic :
			return sess.run( [self.policy_action], feed_dict=feed_dict)[0]
		else :
